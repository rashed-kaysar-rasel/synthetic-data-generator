                                    ...
                                    <tr>
                                        ...
                                        <td class="px-3 py-2 text-slate-600">
                                            @if ($column['isPrimaryKey'])
                                                <span class="inline-flex items-center rounded-full border border-slate-300 px-2 py-0.5 text-xs font-medium text-slate-700">PK</span>
                                            @endif
                                            @if ($column['isForeignKey'])
                                                <span class="ml-1 inline-flex items-center rounded-full border border-slate-300 px-2 py-0.5 text-xs font-medium text-slate-700">FK</span>
                                            @endif
                                            @if (!empty($column['isUnique']))
                                                <span class="ml-1 inline-flex items-center rounded-full border border-slate-300 px-2 py-0.5 text-xs font-medium text-slate-700">UQ</span>
                                            @endif
                                            @if (isset($column['nullable']) && !$column['nullable'])
                                                <span class="ml-1 inline-flex items-center rounded-full border border-slate-300 px-2 py-0.5 text-xs font-medium text-slate-700">NN</span>
                                            @endif
                                        </td>
                                                ...
                                                @foreach ($dataProviders as $group => $providers)
                                                    <optgroup label="{{ ucfirst($group) }}">
                                                        @foreach ($providers as $provider)
                                                            <option value="{{ $group }}.{{ $provider }}">{{ $provider }}</option>
                                                        @endforeach
                                                    </optgroup>
                                                @endforeach
                                            </select>
